...
export default function VendedorPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [sellerInfoRes, sellerListingsRes, allRes] = await Promise.all([
          fetch(`/api/users/${sellerId}`).catch(err => {
            console.error('Seller info fetch failed:', err);
        ...
        if (sellerInfoRes.ok) {
          const sellerData = await sellerInfoRes.json();
          setSeller(sellerData);
        }

        const sellerListData = sellerListingsRes.ok ? await sellerListingsRes.json() : [];
        setListings(Array.isArray(sellerListData) ? sellerListData : []);
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <button 
            onClick={() => router.push('/')}
            className="px-6 py-2 bg-[#2D5A27] text-white rounded-xl font-bold hover:bg-[#1E3D1A] transition-colors"
          >
