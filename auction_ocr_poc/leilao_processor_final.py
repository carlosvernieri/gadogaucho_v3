...
def run_extraction(video_url, auction_id, output_dir, auction_name="santa_ursula"):
    print(f"Iniciando processamento do leilao ID: {auction_id} ({auction_name})")
    video_filename = f"video_{auction_id}.mp4"
    video_path = os.path.join(output_dir, video_filename)
    
    is_lavras = "lavras" in auction_name.lower()
    ...
    while cap.isOpened():
        ...
        if count % int(fps * 3) == 0:
            ...
            # Desvio para Lavras do Sul: usa uma área vertical maior (últimos 35%)
            # para capturar o topo do box de preço que é ligeiramente mais alto
            if is_lavras:
            ...
            if is_lavras:
                current_data = parse_auction_data_lavras(results)
            else:
...
if __name__ == "__main__":
    ...
    run_extraction(args.url, args.id, output_folder, args.name)
